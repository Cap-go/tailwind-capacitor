...
module.exports =(({ addUtilities, theme }) => {
    ...
    Object.keys(themeColors).forEach((key) => {
      ...
      if (typeof value === 'string') {
        hairlineColors[`.hairline-${key}`] = {
          '--c-hairline-color': alphaValue(value),
        };
      } else {
        Object.keys(value).forEach((subKey) => {
          ...
          if (subKey === 'DEFAULT') {
            hairlineColors[`.hairline-${key}`] = {
              '--c-hairline-color': alphaValue(subValue),
            };
          } else {
    ...
    Object.keys(themeDurations).forEach((key) => {
      ...
      hairlineDurations[`.hairline-duration-${key}`] = {
        '--c-hairline-transition-duration': themeDurations[key],
      };
    });

    const common = {
      position: 'absolute',
      backgroundColor: 'var(--c-hairline-color, rgba(0,0,0,0.2))',
      transitionDuration: 'var(--c-hairline-transition-duration)',
      content: '""',
      zIndex: '10',
    };
    ...
    const hairlines = {
      '.hairline-t': {
        '&::before': {
          ...
          transform: 'scaleY(calc(1 / var(--c-device-pixel-ratio, 1)))',
        },
      },
      '.hairline-t-none': {
        '&::before': {
      ...
      '.hairline-l': {
        '&::before': {
          ...
          transform: 'scaleX(calc(1 / var(--c-device-pixel-ratio, 1)))',
        },
      },
      '.hairline-l-none': {
        '&::before': {
